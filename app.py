...
# -----------------------------
# Main Dashboard Layout (Fragment Controlled)
# -----------------------------
@st.fragment
def render_dashboard_content():
    ...
    with col_map:
        ...
        current_lat = float(selected_row["lat"])
        current_lon = float(selected_row["lon"])

        # Center initially on Jacksonville, FL or active point
        m = folium.Map(
        ...
        map_data = st_folium(
            ...
            key="fixed_map_component",
        )

        # Click handling (No manual st.rerun calls!)
        if map_data and map_data.get("last_clicked"):
            ...
            if current_click != st.session_state.last_processed_click:
                ...
                distance_m = dist[0][0] * 6371000  # meters
